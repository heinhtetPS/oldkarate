...
public class EnemyWrestler3 : MonoBehaviour {
	
	public float health = 18, maxhealth = 18;
	...
	public bool burndrop = false;
	private float burnCD = 0;
	private bool hurtdrop = false;
	private float hurtCD = 0;
	public GameObject hurtboxforburn;
	
	public bool slowed = false, gotdmgbybomb = false, gotdmgbydkick = false, dbzmode = false, gravitypulled = false, 
	falling = false, otg = false, balloon = false, stunned = false, grabbed = false, gotdmgbyforce = false, gotdmgbytackle = false;
	...
	public GameObject spawner, burnin, debris;
	public AudioClip gsmashsound;
	
	private float velocityX, velocityY;
	private float gravity = -23;
	
	
	//items
	private int itemroll;
	public GameObject sushi, money, chiball, life, explosion, redscroll;
...
//				canhityou = true;
				...
				if (!stunned && !gravitypulled && dbzmodetimer - Time.deltaTime > 0.6f)
				{
					CancelDBZ(tempstore);
				}
			}
...
//				punkanim.Play("g2stomach");
		...
		//gsmash & bounce
		if (transform.position.y <= -325 && falling && tag == "Hardcore3")
		{
			falling = false;
			otg = true;
		...
		if (gameObject.tag == "Hardcore3" && otg)
		{
...
//			punkanim.Play("Stomacheache");
		...
		if (burndrop && tag != "Dead")
		{
			Debug.Log("droppin shit");
			Instantiate(burnin, new Vector3(transform.position.x, transform.position.y, -120), transform.rotation); 	
			Instantiate(hurtboxforburn, new Vector3(transform.position.x, transform.position.y, -120), transform.rotation); 	
			burndrop = false;
		}
		
		if (hurtdrop && tag != "Dead")
		{	
			Instantiate(hurtboxforburn, new Vector3(transform.position.x, transform.position.y, -120), transform.rotation); 	
			hurtdrop = false;
		}
	...
	void SpawnPowerup () 
	{
		...
		int scrollroll = Random.Range(1, 3);
